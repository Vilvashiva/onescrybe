...
<section class="section services">
  ...
  <div style="margin-left:10%;width:80%">
    <div class="row gx-5 mt-12 items-center lg:mt-0">
      ...
      <div class="mt-6 lg:col-6 lg:order-1 lg:mt-0">
          <p class="modern2">
           With the <span class="high2">clusters identified</span> and video upload patterns visualized over time, we gained valuable insights into how the channel's content 
           has evolved and how different 
           topics resonate with audiences. These <span class="high2">visualizations</span> not only provide a snapshot of the channel's activity and topic diversity but also lay the groundwork for more advanced 
           analyses in the future.<br><br>
           Potential future work could include <span class="high2">sentiment analysis of descriptions</span>, topic overlap across similar channels, and identifying collaboration opportunities within the 
           broader mathematics community.</p><br>
...
<div class="container">
               <div style="margin-left:1%;margin-right:5%;">
                  <h3 style="text-align:center;">📈 Results of These AI Algorithms</h3>
                  <div id="lineChart" style="width: 100%; height: 700px; margin: auto;"></div>
                  <br><br>
                  <div id="pieChart" style="width: 650px; height: 600px; margin: auto;"></div>
                  <span style="margin-bottom:200px;"></span>
                  <br><br><br><br><br><br><br><br><br>
               </div>
               <script>
                  fetch('/js/videolist_with_clusters.json')
                    .then(response => response.json())
                    .then(data => {
                      // Process published dates
                      const publishedDates = data.map(video => new Date(video.publishedAt));
                      const clusters = data.map(video => video.topic_cluster);
                      ...
                      Highcharts.chart('lineChart', {
                        ...
                        title: { text: 'Monthly Video Uploads from MindYourDecisions' },
                        subtitle: { text: 'Zoom to inspect specific periods' },
                      ...
                      const clusterLabels = Object.keys(clusterCounts).map(cluster => `Cluster ${cluster}`);
                      const clusterData = Object.keys(clusterCounts).map(cluster => ({
                        name: `Cluster ${cluster}`,
                        y: clusterCounts[cluster]
                      }));
                  
                      // Pie Chart
                      Highcharts.chart('pieChart', {
                        chart: { type: 'pie' },
                        title: { text: 'Topic Cluster Distribution' },
